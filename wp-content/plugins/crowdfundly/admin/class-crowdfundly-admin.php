...
class Crowdfundly_Admin {
	...
	public function enqueue_scripts() {
		...
		$single_camp_page_slug 			= isset(Crowdfundly_Settings::get()['crowdfundly_option_crowdfundly_single_campaign_slug'])?Crowdfundly_Settings::get()['crowdfundly_option_crowdfundly_single_campaign_slug']:'crowdfundly-single-campaign';
	...
	public function crowdfundly_connect_api() {
		...
		if ( $wp_response['code'] === 200 ) {
			...
			$settings = [
				'crowdfundly_option_api_key' => $api_key,
				'token' => $wp_response['token'],
				'token_expired_at' => time() + $response_body->ttl,
				'email_login' => false,
	...
	public function auto_login_after_session_expired() {
		$crowdfundly_settings =  get_option( 'crowdfundly_settings', null );
		if ( empty( $crowdfundly_settings ) || ! $crowdfundly_settings || !isset( $crowdfundly_settings['token_expired_at'] ) ) return;
		if ( $crowdfundly_settings['token_expired_at'] > time() ) return;
		
		$response = null;
		$token = null;
		if ( $crowdfundly_settings['email_login'] == 1 ) {
			$email = $crowdfundly_settings['email'];
			$password = Crowdfundly_Helper::encrypt_decrypt( 'decrypt', $crowdfundly_settings['chabi'] );
			$request = [
				'body' => [ 'email' => $email, 'password' => $password, 'type' => 'regular' ],
				'timeout' => 12,
			];
			$response = wp_remote_post( Crowdfundly_Api::$post_endpoints['email_login'], $request );
			$response_body = json_decode( $response['body'] );
			$token = $response_body->access_token;
		} else {
			$api_key = $crowdfundly_settings['crowdfundly_option_api_key'];
			$request = [
				'body' => [ 'token' => $api_key ],
				'timeout' => 12,
			];
        ...
        if ( $response_code == 200 ) {
            $crowdfundly_settings['token'] = $token;
			$crowdfundly_settings['token_expired_at'] = time() + $response_body->ttl;

			update_option( 'crowdfundly_settings', $crowdfundly_settings );
			return;
        }
	}
	...
	public function email_login_after_reg($signup_data) {
		...
		if ( $wp_response['code'] === 200 ) {
			$countries_response = wp_remote_get( CROWDFUNDLY_APP_URL . '/api/v1/countries' );
			$currencies_response = wp_remote_get( CROWDFUNDLY_APP_URL . '/api/v1/currencies' );
			...
			$settings = [
				...
				'chabi' => Crowdfundly_Helper::encrypt_decrypt( 'encrypt', $authData['password'] ),
				'countries' => $wp_response['countries'],
				'currencies' => $wp_response['currencies']
			];
			update_option( 'crowdfundly_settings', $settings );
	...
	public function crowdfundly_email_login_auth() {
		...
		if ( $wp_response['code'] === 200 ) {
			...
			$settings = [
				...
				'token_expired_at' => time() + $response_body->ttl,
				'email_login' => true,
				'email' => $email_login_data['email'],
				'chabi' => Crowdfundly_Helper::encrypt_decrypt( 'encrypt', $email_login_data['password'] ),
				'countries' => $wp_response['countries'],
				'currencies' => $wp_response['currencies']
			];
			update_option( 'crowdfundly_settings', $settings );
	...
	public static function fav_icon() {
		...
		if ( $username && $admin_page ) {
			...
			$favicon = ( is_object( $orgs_info ) && isset( $orgs_info->favicon ) ) ? $orgs_info->favicon->source_path : null;
	...
	/**
	 * Plugin version competibility for auto login.
	 */
	function crowdfundly_force_logout() {
		$crowdfundly_settings = get_option( 'crowdfundly_setttings' );
		if ( ! $crowdfundly_settings || empty( $crowdfundly_settings ) ) return;
	
		if ( CROWDFUNDLY_VERSION === '1.1.7' && !isset( $crowdfundly_settings['token_expired_at'] ) ) {
			$login_type = $crowdfundly_settings['email_login'];
			...
			if ( $login_type != 1 ) {
				...
				$response_body = json_decode( $response['body'] );
				$crowdfundly_settings['token'] = $response_body->token;
				$crowdfundly_settings['token_expired_at'] = $response_body->ttl;
				update_option( 'crowdfundly_settings', $crowdfundly_settings );
			} else {
				delete_option( 'crowdfundly_settings' );
			}
