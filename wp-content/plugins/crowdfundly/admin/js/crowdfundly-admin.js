(function( $ ) {
	...
	async function changeToken() {
		const lsToken = localStorage.getItem('auth._token.local');
		if ( ! crowdfundlyAuth || ! lsToken ) return;

		const dbToken = 'Bearer ' + crowdfundlyAuth.bearer_token
		if ( dbToken != lsToken ) {
			await localStorage.setItem('auth._token.local', dbToken);
			await Cookies.set('auth._token.local', dbToken);
