...
class Crowdfundly_Public {
	...
	public function crowdfundly_all_campaign_load_more() {
		...
		$url =  CROWDFUNDLY_APP_URL . '/api/v1/organizations/' . $username . '/active-campaigns/?per_page=' . $per_page . '&page=' . $current_page;
	...
	public static function campaign_donation_status($campaign, $notice) {
		$status = true;

		$camp = $campaign;

		$crowdfundly_settings = get_option( 'crowdfundly_settings', null );
		$crowdfundly_option_wc_payment = isset( $crowdfundly_settings['crowdfundly_option_wc_payment'] ) ? $crowdfundly_settings['crowdfundly_option_wc_payment'] : 0;
		$available_woo_gateways = function_exists( 'WC' ) ? WC()->payment_gateways->get_available_payment_gateways() : [];
		if ( $crowdfundly_option_wc_payment == 1 && empty( $available_woo_gateways ) ) {
			$status = false;
		}

		if ( $camp->target_amount_raised == 1 ) {
			$status = false;
		}
		if ( isset( $camp->validity_countdown_days ) && $camp->validity_countdown_days == 0 ) {
			$status = false;
		}
		if ( ! empty( $notice ) ) {
			$status = false;
