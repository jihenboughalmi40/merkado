...
class Easyjobs_Admin {
	...
	public function enqueue_scripts() {
       ...
       /****** new scripts ********/
        ...
        if(defined('EASYJOBS_DEV_SCRIPT') && EASYJOBS_DEV_SCRIPT){
            wp_register_script('easyjobs-react',EASYJOBS_ADMIN_URL.'assets/dist/js/easyjobs.js',['wp-i18n'], filemtime(EASYJOBS_ADMIN_DIR_PATH.'assets/dist/js/easyjobs.js'), true);
        }else{
            wp_register_script('easyjobs-react',EASYJOBS_ADMIN_URL.'assets/dist/js/easyjobs.min.js',['wp-i18n'], filemtime(EASYJOBS_ADMIN_DIR_PATH.'assets/dist/js/easyjobs.min.js'), true);
        }
    ...
    public function sync_data()
    {
        if ( ! isset( $_POST['nonce'] ) && !
            ...
            echo Easyjobs_Helper::get_error_response('Invalid request');
            wp_die();
        }
        $settings = EasyJobs_DB::get_settings();
        $updated = false;
        if(!empty($settings['easyjobs_api_key'])){
            ...
            if($updated){
                echo Easyjobs_Helper::get_success_response('Data synced successfully');
            }else{
                echo Easyjobs_Helper::get_error_response('Data synced failed');
            }
        }else{
            echo Easyjobs_Helper::get_error_response('Api not connected');
        }
    ...
    {
        $analytics_connected = Easyjobs_Api::post('analytics_url',null, [
