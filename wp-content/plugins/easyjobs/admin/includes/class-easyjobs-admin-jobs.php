...
class Easyjobs_Admin_Jobs
{
    ...
    public function delete_job()
    {
        ...
        $response = Easyjobs_Api::post('delete_job', absint($_POST['job_id']), []);
        if(Easyjobs_Helper::is_success_response($response->status)){
            $this->delete_job_page(absint($_POST['job_id']));
            echo Easyjobs_Helper::get_success_response(__('Job deleted successfully', 'easyjobs'));
        }else{
            echo Easyjobs_Helper::get_error_response(__('Failed to deleted job, please try again or contact support','easyjobs'));
        }
    ...
    public function get_initial_job_templates()
    {
        ...
        if(isset($_POST['industry_id']) &&  !empty($_POST['industry_id'])){
            if(trim($_POST['industry_id']) == 'all'){
                $industry = '';
            }else{
                $industry = abs($_POST['industry_id']);
            }
        }else{
        ...
        $initial_templates = Easyjobs_Api::get('job_templates',[
        ...
        if(Easyjobs_Helper::is_success_response($initial_templates->status)){
            ...
            echo Easyjobs_Helper::get_success_response('Successfully get templates', $response_data);
        }else{
            echo Easyjobs_Helper::get_error_response('Unable to get job templates, please try again');
        }
        wp_die();
    }
    ...
    private function delete_job_page($jobId)
    {
