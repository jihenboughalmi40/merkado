<?php
/**
 * BetterDocs_Plugin_Usage_Tracker
...
/**
 * Main SDK for BetterDocs_Plugin_Usage_Tracker.
 */
if( ! class_exists('BetterDocs_Plugin_Usage_Tracker') ) :
	class BetterDocs_Plugin_Usage_Tracker {
		/**
		 * WP Insights Version
		 */
		...
		public function send_data( $body ) {
			...
			if( isset( $request ) && ! is_wp_error( $request ) && $request['response']['code'] == 200 ) {
				return true;
			}
		...
		public function notice() {
			...
			$output = '';
			$output .= '<div class="notice notice-info updated put-dismiss-notice notice-'. $this->plugin_name .'">';
		...
		public function deactivation_reasons() {
			$form = array();
			$form['heading'] = __( 'Sorry to see you go', 'betterdocs' );
			$form['body'] = __( 'Before you deactivate the plugin, would you quickly give us your reason for doing so?', 'betterdocs' );
			...
			<script type="text/javascript">
				jQuery(document).ready(function($){
					$("#wpinsights-goodbye-link-<?php echo $class_plugin_name; ?>").on("click",function(){
						...
						$(".wpinsights-goodbye-form-wrapper-<?php echo $class_plugin_name; ?> #wpinsights-goodbye-form").html( '<?php echo $html; ?>' + '<div class="wpinsights-goodbye-form-footer"><div class="wpinsights-goodbye-form-buttons"><a id="wpinsights-submit-form-<?php echo $class_plugin_name; ?>" class="wpinsights-submit-btn" href="#"><?php _e( 'Submit and Deactivate', 'betterdocs' ); ?></a>&nbsp;<a class="wpsp-put-deactivate-btn" href="'+url+'"><?php _e( 'Just Deactivate', 'betterdocs' ); ?></a></div></div>');
